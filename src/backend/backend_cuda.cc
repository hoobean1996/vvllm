                         ...
                         std::size_t M, std::size_t N, std::size_t K)
{
    if (fp16_)
    {
        ...
        cuda_cublas_hgemm(out, d_tmp_fp16_in_, d_w, M, N, K, 0.0f);
        if (d_bias)
        {
            // Bias is FP16, need FP32 bias for add_bias on FP32 output
            // Use cache_weight for FP32 bias instead
            float* d_bias_fp32 = static_cast<float*>(cache_weight(bias, N * sizeof(float)));
            cuda_add_bias(out, d_bias_fp32, M, N);
        }
        return;
    }
                              ...
                              const float* residual, std::size_t M, std::size_t N, std::size_t K)
{
    ...
    if (fp16_)
    {
        ...
        cuda_cublas_hgemm(out, d_tmp_fp16_in_, d_w, M, N, K, 1.0f);
        if (bias)
        {
            float* d_bias = static_cast<float*>(cache_weight(bias, N * sizeof(float)));
            cuda_add_bias(out, d_bias, M, N);
        }
        return;
    }
