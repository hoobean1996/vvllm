...
Qwen2Model::Qwen2Model(const ModelConfig& config, Backend& backend)
    : config_(config),
      backend_(backend),
                              ...
                              bool quantize)
{
    ...
    // Helper to set a LinearWeight field, optionally quantizing.
...
Tensor Qwen2Model::forward(const std::vector<int>& token_ids, std::size_t pos,
                           KVCache& kv_cache)
{
